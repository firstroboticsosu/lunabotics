...
// updated apr 2024 by Noah Charlton - handle error conditions
...
int AMS_5600::detectMagnet()
{
  ...
  int magStatus;
  if(readOneByteWithTimeout(_addr_status, 1000, &magStatus)) {
...
bool AMS_5600::readOneByteWithTimeout(int in_adr, int timeout_ms, int *result) {
  Wire.beginTransmission(_ams5600_Address);
  Wire.write(in_adr);
...
bool AMS_5600::readTwoByteTogetherWithTimeout(int in_adr, int timeout_ms, int *result) {
  // Send request
  Wire.beginTransmission(_ams5600_Address);
  Wire.write(in_adr);
...
bool AMS_5600::tryGetRawAngle(int *angle) {
  int result;
  if(readTwoByteTogetherWithTimeout(_addr_raw_angle, 1000, &result)) {
    *angle = result; 
    return true;
  }
